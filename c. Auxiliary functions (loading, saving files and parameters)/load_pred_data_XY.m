...
% License : 3-clause BSD License
    ...
    data_all_markers = data_all_markers(:,1:pred_par.tmax_pred);
        % if tmax_pred < the total amount of time steps in the variable 'data_all_markers' then we crop that variable to decrease the calculation time
    ...
    if pred_par.NORMALIZE_DATA
        ...
        [ ~, Mu, Sg ] = standard_scores( data_all_markers(:,1:pred_par.tmax_training));
        data_all_markers = normalize_from_computed_stats( data_all_markers, Mu, Sg);
    else
        Mu = 0;
        Sg = 0;
    end
    ...
    for j =1:M
        Ttemp = data_all_markers(:,j:(j+k-1)); % size (1+k*data_dim, k)
        X(2:(1+k*data_dim),j) = Ttemp(:);
    end
   
    Y = double(data_all_markers(:, (k+h):(M+k+h-1)));
