...
% We make the assumption that the data to be predicted consists in the position of 3D points.
% This function :
%   - computes the mean prediction error, the root-mean square, the maximum error, the associated 95% mean confidence intervals, the number of numerical erros,
...
% License : 3-clause BSD License
    ...
    load(path_par.time_series_data_filename, 'data_all_markers');
    data_all_markers = data_all_markers(:,1:pred_par.tmax_pred);
    [data_dim, ~] = size(data_all_markers);
    nb_pts = data_dim/3; % because we are predicting the 3D position of points.
    true_pts_pos = zeros(pred_par.tmax_pred, nb_pts, 3);
    
    data_all_markers = data_all_markers.';
    true_pts_pos(:,:,1) = data_all_markers(:,1:nb_pts);                %x coordinates of the points 
    true_pts_pos(:,:,2) = data_all_markers(:,(nb_pts+1):(2*nb_pts));   %y coordinates of the points 
    true_pts_pos(:,:,3) = data_all_markers(:,(2*nb_pts+1):(3*nb_pts)); %z coordinates of the points 
    
    mu_true = zeros(3, nb_pts); % used for calculating nrmse
    ...
    num_error_idx_vec = any(isnan(eval_results.mean_err),2);
    ...
    if (beh_par.SAVE_PREDICTION_PLOT)
        RNNmeanloss = mean(RNNloss_all_runs, 2);
        SAVE_ONLY = false;     
        ...
        plot_pred_error( RNNmeanloss, disp_par, pred_par, path_par, filename_suffix, title_str, SAVE_ONLY )
    end
